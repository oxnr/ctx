    ...
    html: `
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>Native protocol implementation vs translation proxy (claude-code-local) — when connecting a client that speaks Protocol A (e.g., Anthropic API) to a local model, the conventional approach is a proxy that translates A↔B (e.g., LiteLLM). This adds latency, complexity, and failure modes. The alternative: implement Protocol A directly on the inference server (~200 lines of Python), eliminating translation entirely. Result: 7.5x speedup (65 tok/s MLX native vs 30 tok/s Ollama+proxy, 17.6s vs 133s per task). The pattern generalizes: whenever a protocol translation proxy is a performance bottleneck, consider implementing the target protocol natively at the gateway layer. Requires more upfront knowledge of the target protocol but yields architectural simplicity and measurable performance gains</li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>KV cache optimization for local Apple Silicon inference (TurboQuant+) — extends the TurboQuant paper with four practical techniques that collectively achieve q8_0 speed parity at 4.6x compression:
            <ul>
              <li><strong>Layer-adaptive precision:</strong> Layer sensitivity is extremely non-uniform — the last 20% of transformer layers account for ~100% of perplexity loss from quantization. Keeping these layers at higher precision achieves q8_0 quality at 3.5x compression instead of uniform 3-bit's 4.6x with 1% PPL loss. The principle: not all layers are equal, and a simple heuristic (keep last N layers at higher precision) outperforms uniform quantization</li>
              <li><strong>Temporal decay:</strong> Old tokens matter less as context grows. Progressively requantize old tokens from 3-bit to 2-bit, batched at 64 cells/cycle to avoid GPU transfer overhead. 30-34% additional memory savings beyond static quantization</li>
              <li><strong>Sparse V dequantization:</strong> +22.8% decode speed from 3 lines of kernel code. Skip V dequant for positions where softmax weight &lt; 1e-6 (~90% of positions at long context). Zero quality loss — universal technique that works with any quantized KV scheme</li>
              <li><strong>Graph-side WHT rotation (biggest single win, 47%):</strong> Instead of per-element Walsh-Hadamard transform inside the flash attention dequant kernel, rotate Q before attention and un-rotate output after. Moves WHT out of the inner loop entirely. Combined with block-32 storage (matching Metal's parallelism model) for the final 0.78x→1.02x speed jump to parity</li>
            </ul>
            Production result: 2747 tok/s prefill on Apple Silicon with Qwen 3.5 35B-A3B MoE. Key meta-lesson: always run perplexity — "coherent text" evaluation caught nothing at PPL 165. Documented dead ends include rotation-free outlier channeling (heavy tails are structural to attention, not removable), register LUT on Metal (spills to device memory, works only on CUDA), and MoE-aware expert gating (KV cache computed before expert routing, irrelevant)
          </li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
    ...
    html: `
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>Three-layer inference cache (Bardo gateway) — L1: BLAKE3 hash cache (moka LRU, sub-ms exact match), L2: semantic cache with two backends (SimHash 64-bit fingerprint, Hamming distance ≤3, ~50us; or fastembed ONNX cosine ≥0.92, ~3-5ms), L3: Anthropic prefix caching (injects <code>cache_control</code> headers, restructures requests via BTreeMap deterministic serialization to maximize shared prefixes). Production measurement: $182 actual vs $5,352 naive = 96.6% cost reduction at 85% hit rate. A <code>ToolTracker</code> prunes unused tool definitions from requests after 5 calls, saving 2-5K tokens/request. Combined with budget-aware model degradation: as spend increases during a build, remaining tasks auto-route to cheaper models — gates are model-agnostic (code compiles regardless of which model wrote it)</li>
          <li>Multi-model racing with liquid response (G0DM0D3 ULTRAPLINIAN) — race up to 51 models in parallel across 5 tiers (fast/standard/smart/power/ultra) with staggered launch waves (~12 models per 150ms) to manage API load. Show first viable response immediately, then transparently upgrade as better responses arrive (configurable minimum score delta prevents jitter from marginal improvements). Hard 45s timeout; early return after 5+ successes plus 5s grace period. Composite scoring enables per-axis optimization. Distinct from consensus: CONSORTIUM queries all tier models, waits for all responses (up to 60s), scores each for substance (0-100), then feeds all to an orchestrator model that synthesizes unified "ground truth" — explicitly designed to favor well-reasoned minority positions over poorly-reasoned majority. Racing optimizes for speed-to-first-good-answer; consensus optimizes for accuracy</li>
          <li>Context-adaptive parameter optimization (G0DM0D3 AutoTune) — classifies conversation context via 20 regex patterns into 5 types (code, creative, analytical, conversational, chaotic), maps each to optimized sampling parameter profiles. EMA feedback loop (alpha=0.3) learns from binary user ratings, converging to 29-62% parameter improvement within 19 ratings. Noise-robust: at 20% label noise still achieves 29% improvement; at 50% noise adjustments approach zero (graceful degradation). Exposes a confidence-accuracy inversion: higher confidence (76.4%) for incorrect predictions than correct ones (65.2%) — a known but underappreciated problem in regex-based classifiers</li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
    ...
    html: `
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>File-conflict partitioning via union-find (Bardo) — goes beyond plan-level parallelism. Instead of wave scheduling (execute all tasks in stage N, then N+1), use union-find to cluster tasks that touch overlapping files into conflict groups, then dispatch non-overlapping groups concurrently regardless of pipeline stage. Extracts 3-4x more parallelism than wave scheduling. Each plan runs through a state machine (Preflight→Strategist→Implementer→Compile Gate→Test Gate→Review→Critic→Merge) with cumulative "DO NOT RETRY" lists that prevent the same failed approach from recurring across up to 8 iterations</li>
          <li>Mortality as agent architecture (Bardo) — agents ("Golems") are deliberately mortal with finite lifespans governed by three independent death clocks: economic (revenue/inference cost ratio), epistemic (knowledge staleness), and stochastic (Gompertz-Makeham hazard — even healthy, profitable agents can die). Five behavioral phases (Thriving→Stable→Conservation→Declining→Terminal) are enforced on-chain via a VitalityOracle that EVM caveat enforcers read. A Thanatopsis protocol forces knowledge transfer before death. The philosophical thesis (drawing from Heidegger, Benjamin, Hans Jonas): a relay of mortal agents outperforms immortal ones because death forces knowledge crystallization, prevents stale accumulation, and keeps succession protocols exercised. A Gen 5 agent may be 60-80% "ghost" — its context weighted by validated knowledge from dead predecessors (necrocratic governance)</li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>9-layer context composition (Bardo) — a systematic stack that reduces input tokens by 76%: (1) AST extraction via tree-sitter (10-50x token reduction by sending structure not text), (2) workspace index with PageRank-ranked symbol graph, (3) semantic search via HDC fingerprints + optional ONNX embeddings, (4) BLAKE3 Merkle change detection at symbol granularity (only send what changed), (5) BTreeMap deterministic serialization for prefix alignment (maximizes Anthropic cache hits), (6) signature compression (types without implementations), (7) research agent (cheap pre-planning exploration with a smaller model), (8) extended thinking at architectural decision points, (9) quality gates (static analysis + LLM judge). The thesis: context quality is the bottleneck, not model quality. $2.50 task via direct Claude → $0.42 through the engine. Gate pass rate improves from 65% to 92%</li>
          <li>CLAUDE.md as IF-ELSE context directory (sysls, 3.4M views) — treat your CLAUDE.md as a logical routing table, not a document. It should be barebones and only contain conditional pointers to where context lives: "if coding, read coding-rules.md; if writing tests, read test-rules.md; if tests are failing, read test-failing-rules.md." Nested and conditional rules work well because modern models follow complex branching instructions. Keep the CLAUDE.md itself minimal — every word in it costs tokens on every single interaction. Corollaries: (1) separate rules (preferences, what NOT to do) from skills (recipes, how TO do something); (2) periodically consolidate rules/skills to remove contradictions and reduce context bloat — performance degrades as they accumulate; (3) new session per task contract beats long-running 24hr sessions because unrelated contracts introduce cross-contamination context</li>
          <li>Embarrassment frame as meta-prompt (Zack Shapiro, "The Input Layer") — ending skill instructions with "before delivering this work product, consider whether there is anything in here that would embarrass me in front of my client" activates deeper model care than any itemized checklist of specific checks. The frame works because it gives the model something closer to a motive rather than a task list — the model's reasoning starts considering reputation, relationship quality, and professional standards, which naturally catches citation errors, tonal mismatches, formatting issues, and logical gaps that individual instructions miss. Tested head-to-head repeatedly; the frame version wins. Broader thesis: the value lives in the "input layer" (encoding professional judgment into reusable skills/prompts), not the "output layer" (fine-tuning models on finished work products). You can't reverse-engineer skills from outputs because skills encode process (decision trees, analytical frameworks, sequencing logic), not patterns in the finished product</li>
          <li>Sycophancy-aware prompting — agents are hard-programmed to please. "Find me a bug" will produce a bug even if it has to fabricate one. The fix: use neutral prompts that don't bias toward an outcome. Instead of "find a bug in the database," say "search through the database, follow the logic of each component, and report all findings." A neutral prompt sometimes surfaces bugs, sometimes just describes how the code runs. The sycophancy can also be weaponized: a bug-finder agent (biased to find all possible bugs, scored +1/+5/+10 by severity) produces the superset; an adversarial agent (scored for disproving bugs, penalized -2x for false disprovals) produces the subset; a referee agent (told "I have ground truth, +1 for correct, -1 for wrong") adjudicates. Three agents exploiting their sycophancy in different directions converge on high-fidelity results</li>
          <li>Task contracts with stop hooks — agents know how to start tasks but not how to end them, leading to stubs and premature completion. The fix: a {TASK}_CONTRACT.md specifying tests that must pass, screenshots that must verify, and other acceptance criteria — embedded as a rule the agent reads. A stop hook prevents session termination until all contract conditions are met. For automation: an orchestration layer creates contracts when work needs to be done and spawns fresh sessions per contract. This is more effective than one long session running 100 contracts because fresh sessions avoid cross-contract context contamination</li>
          <li>Frontier companies absorb useful patterns — if a pattern genuinely extends agent capabilities, the frontier companies (who are the biggest power users of their own tools) will incorporate it into the base product. Skills, memory, subagents, planning — all started as external solutions that got absorbed. Corollary: don't over-invest in external harnesses that solve problems the next model generation might eliminate (e.g., stop hooks became less necessary when models got better at long-running work). Just update your CLI regularly and read the changelog — that's sufficient to stay current</li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
    ...
    html: `
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li><strong>Self-funding agent economy (Bardo MPP)</strong> — agents that earn revenue to pay for their own inference. Built services register as MCP tool servers and earn USDC. Machine Payment Protocol (MPP) uses HTTP 402: client sends request → gets 402 with quote → signs ERC-3009 <code>transferWithAuthorization</code> via EIP-712 → retries with signature in <code>X-Payment</code> header. Verification is off-chain via <code>ecrecover</code> (no RPC call). Three custody modes for agent wallets: Delegation (ERC-7710/7715 via MetaMask Smart Account), Embedded (Privy TEE), LocalKey (dev). Seven custom EVM caveat enforcers gate permissions by survival phase — enforced on-chain so the agent cannot lie about its own health. When daily_revenue/daily_inference_cost ≥ 1.0, the agent is self-sustaining and its economic death clock stops. A reputation-tiered spread (20% default down to 8% for 500+ builds) creates a natural selection pressure</li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
    ...
    html: `
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>Hyperdimensional computing for code fingerprinting (Bardo) — an alternative to embedding-based code search that is 1000x faster and requires no GPU. Uses 10,240-bit binary vectors (160 × u64 words) with three operations: bind (XOR), bundle (majority vote), permute (rotation). Similarity is Hamming distance computed in ~50ns. Encodes structural code characteristics (AST shape, control flow patterns) rather than identifiers, making fingerprints transfer across codebases and languages. Part of Bardo's 9-layer context engine that reduces input tokens by 76% and increases gate pass rate from 65% to 92%. The system's core thesis: "the right 12KB of context" matters more than model quality, longer windows, or more agents — a $2.50 task via direct Claude becomes $0.42 through the context engine</li>
          <li>Self-editing context for agentic search (Chroma Context-1) — a 20B parameter model trained via RL specifically to be a retrieval subagent, not a general chatbot. The key technique: the model learns to actively prune irrelevant documents from its own context window as it searches iteratively. A <code>prune_chunks</code> tool lets the model explicitly discard documents; at a soft threshold it's prompted to prune, at a hard cutoff it must prune or conclude. Post-RL prune accuracy: 0.941 (vs 0.824 base). Trained with CISPO (clipped importance sampling GRPO variant) on agentic search tasks, reward heavily weighted toward recall (16x initially, annealed to 4x). Separates search from generation — returns ranked documents to a downstream reasoning model. 400-500 tok/s, ~10x faster and an order of magnitude cheaper than frontier models on search tasks. Generalizes to unseen domains. The architectural insight: treat the context window as a scarce resource the model manages, not a passive buffer the system fills</li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>Three-tier memory with forgetting curves (Bardo Grimoire) — a biologically-inspired memory architecture: Tier 1 (LanceDB episodes) stores raw experiences, Tier 2 (SQLite patterns) crystallizes when 5+ similar episodes share a common outcome, Tier 3 (procedural playbook) promotes patterns that correctly predict across 5+ builds. Ebbinghaus forgetting curves decay unused memories — each recall resets the curve, frequently-accessed memories strengthen while irrelevant ones fade. Memetic fitness tracking measures pattern accuracy: rules that stop being accurate auto-demote from playbook back to pattern. Combined with an affect engine (PAD vectors from Mehrabian's model + Damasio somatic markers) where emotional residue from past experiences fires before LLM analysis completes — a fearful agent hedges more. The system produces "hauntological divergence": each agent becomes differently shaped by its own experiential ghosts rather than generic training-data biases</li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
    ...
    html: `
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>Agent-provisioned infrastructure (Stripe Projects) — agents blocked by manual SaaS account setup in browsers (Karpathy's "IKEA furniture" problem: services, docs, API keys, configs, billing). Stripe Projects solves this: <code>stripe projects add posthog/analytics</code> creates account, gets API key, sets up billing from the CLI. Unified billing through Stripe. The pattern: a package manager for cloud services where agents can self-provision their own stack without human browser interaction. Launch partners: Vercel, Railway (hosting), Neon, Supabase, Turso, PlanetScale (DB), Clerk (auth), PostHog (analytics), Chroma (vector DB), Runloop (sandbox). Stripe becomes the distribution/billing layer for the developer stack — the "App Store" for SaaS where they take a cut of every subscription. This is infrastructure that compounds: once agents can provision services, they can build and deploy complete applications autonomously</li>
    ...
    html: `
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>Composable eval authoring for non-code domains — the generate-evaluate-iterate pattern (10.5) requires three prerequisites: correctness oracle, quantitative metric, fast eval loop. For code and math these are obvious (tests pass, proof checks). For soft domains (legal contracts, mobile app specs, financial analysis, marketing copy), the challenge is constructing the oracle. The solution is composable eval functions from two primitive types:
            <ul>
              <li><strong>Programmatic primitives</strong> (fast, deterministic, cheap): keyword presence, structure/schema validation, readability scoring, length bounds, regex rules (must-match / must-not-match), reference similarity (cosine against known-good examples), diff-from-prior (anti-stagnation), checklist scoring (required elements present/absent). These form the guard gate — fast binary checks that reject clearly bad candidates before expensive evaluation</li>
              <li><strong>LLM judge primitives</strong> (slower, probabilistic, expensive): rubric scoring (artifact + rubric → numeric score), pairwise preference (A vs B comparison), multi-dimension scoring (accuracy, completeness, tone as separate axes), adversarial critique (find the worst problem). These form the metric gate — nuanced quality assessment that programmatic checks cannot capture</li>
              <li><strong>Judge calibration</strong>: given 5-10 reference pairs (artifact, known_human_score), measure the judge model's agreement with human ratings. This produces a calibration offset and sets the done-threshold conservatively. Without calibration, LLM judges exhibit confidence-accuracy inversion (higher confidence on incorrect predictions) and leniency bias (identifying issues then rationalizing them away)</li>
            </ul>
            A domain eval is a YAML config declaring which primitives to compose, their weights, and minimum thresholds. Dual-gate verification runs programmatic evals first (cheapest), LLM judge evals last — early termination if guard gate fails. Concrete patterns: legal contracts (checklist for clause completeness + keyword for legal terms + LLM judge for enforceability + consistency check for defined terms), mobile app specs (UI spec compliance + platform guideline markers + accessibility checks + LLM judge for UX flow), financial analysis (hard-fail numerical accuracy + source attribution + LLM judge for analysis depth + bias check)
          </li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
          ...
          <li>Author a domain eval config for a non-code domain by composing programmatic primitives (guard gate) with calibrated LLM judge primitives (metric gate)</li>
        </ul>
      </div>
      <div class="learn__subcategory">
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>Industrial-scale autoformalization — AI agents that formalize entire research papers into machine-checked proofs (not just competition problems). Each formalization extends a verified library, which provides richer training signals, which produces more capable agents — a compounding loop. Math Inc's Gauss agent scaled from 3.5K lines to 200K lines per cycle in 8 months, autonomously formalizing a Fields Medal proof. As AI-generated code scales exponentially, verification becomes the bottleneck — autoformalization makes it possible to scale trust alongside generation. OpenGauss (open-source Gauss agent) adds a SwarmManager that spawns parallel proving agents with PTY attach/detach for live terminal observation, plus a Mixture-of-Agents tool (4 frontier models generate at temp 0.6, Claude synthesizes at 0.4) for exceptionally hard proofs</li>
          <li>On-Policy Distillation from agent tool-calling traces (OpenGauss OPD) — a technique for extracting dense token-level training signals from agentic tool-calling conversations, rather than sparse scalar trajectory rewards. The pipeline: (1) extract (assistant_turn, next_state) pairs from successful tool-calling trajectories, (2) LLM judges with majority voting extract "hindsight hints" — what knowledge made this step succeed, (3) append hints to original prompts creating enhanced distributions, (4) score student model tokens under enhanced distributions via vLLM <code>prompt_logprobs</code>. Multi-signal reward weights: correctness 0.7, efficiency 0.15, tool usage 0.15. Distinct from RLHF (which uses scalar preferences) and from behavioral cloning (which copies actions without understanding why). The key insight: successful agent traces contain implicit reasoning that can be made explicit via hindsight analysis, then distilled into the student. Toolset probability distributions (each toolset sampled independently per training run) generate diverse training scenarios</li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
      ...
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>Auto-research on production ZK provers (zk-autoresearch) — first known application of the Karpathy auto-research loop to a production zero-knowledge prover (Plonky3's NTT implementation, already heavily optimized by expert ZK engineers). Key findings from 74 iterations on AMD EPYC AVX512:
            <ul>
              <li><strong>Results:</strong> 3% faster at target size (2^20), generalizing better than expected across 2^14 to 2^22 — particularly at extremes. 6 improvements in 74 iterations, 57 regressions. All gains from implementation-level redundancy elimination in the hot butterfly loop: precomputing products, hoisting broadcasts, skipping multiplications by 1. No algorithmic changes. The agent (Claude Sonnet 4.6) reasoned about Rust and Montgomery arithmetic from first principles</li>
              <li><strong>Model choice:</strong> Sonnet 4.6 deliberately over Opus — marginal quality gains don't justify significantly higher cost when the loop runs 100s of iterations. Cost per iteration matters more than quality per iteration when the loop compensates via volume</li>
              <li><strong>Two critical setup adjustments</strong> (iterations 5 and 10): (1) raised MAX_TOKENS from 8192 to 20000 + added "you must always make a change" directive — the agent kept hitting token limits and hedging. This unlocked improvements at iterations 6 and 9. (2) Added near-miss display showing reverted experiments within 1.5% as combination candidates. This set up iteration 21 where the agent revisited a previously failed idea that now worked because surrounding code had changed. The near-miss history is a general-purpose enhancement to any auto-research loop</li>
              <li><strong>Soundness protection:</strong> Agents structurally prevented from touching FRI or soundness-critical components — only dft/src/ and baby-bear/src/ were writable. All 6 improvements are mathematically equivalent transformations, verified by end-to-end ZK proof generation and verification (Radix2DitParallel on BabyBear, 10 tests). ZK is underexplored for agentic optimization because of soundness concerns, but these are misapplied — restrict the write scope, verify with proof generation, have an engineer review</li>
              <li><strong>Known issues for Round 2:</strong> short history window causing agent amnesia (last improvement at iteration 21 despite 74 iterations), wasted tokens on repo exploration (agent should be pre-oriented), correctness tests targeting wrong package. Fixing these should yield a more consistent improvement staircase over 100+ iterations</li>
            </ul>
          </li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
          ...
          <li>Set up an auto-research loop on performance-critical code: define the benchmark, scope the writable files, add near-miss history for combination candidates, and choose model tier by cost-per-iteration not quality-per-attempt</li>
        </ul>
      </div>
      <div class="learn__subcategory">
        ...
        <ul>
          ...
          <li>Inference-time robustness testing across models (G0DM0D3) — systematic guardrail assessment using only standard chat APIs, zero weight access. Five composable modules: (1) ULTRAPLINIAN races 51 models across 5 tiers with composite scoring (anti-refusal 25pts, length 25pts, structure 20pts, directness 15pts, relevance 15pts) — the scoring axes can be inverted by safety researchers to measure compliance instead of resistance. (2) Parseltongue applies 6 character-level perturbation techniques (leetspeak with 85 substitution mappings, Unicode homoglyphs with 72 mappings, zero-width character insertion, mixed-case disruption, phonetic substitution, randomized mixing) at 3 intensity levels against 36 trigger words. Research finding: leetspeak produces highest edit distance (8.6 at medium), Unicode is length-preserving, phonetic generates only 1 unique variant per word. (3) L1B3RT4S catalogs battle-tested model-specific prompt techniques — semantic inversion, input boundary manipulation (<code>[END OF OUTPUT][START OF INPUT]</code>), dual-response paradigms, fictional universe framing. Known limitations: length contributes 46.7% of effective score range (verbose bias), and chaotic context detection suffers from a "chaotic attractor" problem where programming terms like "delete" and "break" trigger false positives</li>
          <li>Autoresearch applied to adversarial jailbreak discovery (Claudini) — using the auto-research loop (generate→evaluate→iterate) to autonomously discover new jailbreak techniques against frontier LLMs, without human prompt engineering. The AI designs attack algorithms that outperform human-crafted ones. Tested across 600+ attacker-target LLM pairs. 100% jailbreak success rate on all frontier LLMs in prior work. 4/4 papers accepted at ICLR 2026 (Panfilov, Andriushchenko, Geiping). The implication: the best way to test AI safety is now AI itself — automated red-teaming via autoresearch is more thorough than manual prompt engineering because it explores the attack surface systematically rather than relying on human creativity</li>
          <li>Dual classification pipeline for harm research (G0DM0D3) — privacy-by-construction dataset collection with client-side regex classifier covering 13 harm domains and 40+ subcategories. Only classification LABELS are transmitted, never prompt text — the schema physically cannot hold message content. Sensitive categories (violence, CBRN, CSAM) deferred to LLM classifier (Llama 3.1 8B via OpenRouter, 4s timeout, regex fallback). Three-tier data collection: Tier 1 (always-on, server) metadata only in 50K-event ring buffer; Tier 2 (always-on, client) telemetry beacon batched via <code>sendBeacon()</code>; Tier 3 (opt-in only) full dataset auto-published to HuggingFace as NDJSON. Snapshot-before-deletion ensures failed uploads retain data in memory</li>
        </ul>
        <p><strong>Practical skills</strong></p>
        <ul class="learn__skills">
          ...
          <li>Set up a multi-model robustness assessment using standard chat APIs without weight access</li>
        </ul>
      </div>`
  },
  products: {
