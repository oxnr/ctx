window.VC_DATA = {
  layers: [
    ...
    { id: "harness", index: "06", label: "Harness", shortDesc: "Agent runtime infrastructure, sandboxes, verification", subcategories: [
      { id: "execution-environments", label: "Execution Environments", desc: "Sandboxes, containers, isolated runtimes" },
      { id: "state-continuity", label: "State & Continuity", desc: "Progress files, session bridging, durable execution" },
      { id: "tool-infrastructure", label: "Tool Infrastructure", desc: "Tool registries, bash/code exec, MCP runtime" },
      { id: "verification-loops", label: "Verification Loops", desc: "Write-test-fix cycles, self-verification" },
      { id: "context-management", label: "Context Management", desc: "Compaction, progressive disclosure, smart zone optimization" },
      { id: "constraints", label: "Constraints & Linting", desc: "Architectural linters, structural tests, dependency enforcement" }
    ]},
    { id: "orchestration", index: "07", label: "Orchestration", shortDesc: "Multi-agent coordination, workflows, oversight", subcategories: [
      ...
      { id: "multi-agent", label: "Multi-Agent", desc: "Agent teams, delegation, handoffs, specialization" }
    ]},
    { id: "context", index: "08", label: "Context", shortDesc: "Vector DBs, RAG, embeddings, memory", subcategories: [
    ...
    { id: "integrations", index: "09", label: "Integrations", shortDesc: "MCP, IDE agents, tool connectors", subcategories: [
    ...
    { id: "eval", index: "10", label: "Eval & Safety", shortDesc: "Evals, observability, guardrails", subcategories: [
      ...
      { id: "ai-red-teaming", label: "AI Red Teaming", desc: "Autonomous adversarial testing of AI systems" }
    ]},
    { id: "products", index: "11", label: "Products", shortDesc: "End-user AI applications", subcategories: [
  ...
  tools: [
    ...
    { id: "nemotron-cascade", name: "Nemotron-Cascade 2", desc: "NVIDIA 30B MoE (3B active) achieving IMO/IOI/ICPC gold via Cascade RL (domain-sequential RL stages) + Multi-Domain On-Policy Distillation (MOPD) consolidating specialist checkpoints into one model", type: "open-weight", url: "https://research.nvidia.com/labs/nemotron/", github: null, directLayers: ["pretrain", "posttrain"], indirectLayers: ["inference"], yc: null, subcategories: ["foundation-models", "alignment"]},
    { id: "qwen3-coder", name: "Qwen3-Coder", desc: "Alibaba's agentic coding model family — flagship Next variant uses hybrid DeltaNet+Attention MoE (75% linear attention, 512 experts, 3B active from 80B total), matches Sonnet 4.5 on SWE-bench Pro, trained with agentic RL on 20K parallel environments", type: "open-weight", url: "https://github.com/QwenLM/Qwen3-Coder", github: "https://github.com/QwenLM/Qwen3-Coder", directLayers: ["pretrain"], indirectLayers: ["inference"], yc: null, subcategories: ["foundation-models"] },
    { id: "qwen-code", name: "Qwen Code", desc: "Open-source terminal AI agent for Qwen3-Coder — adapted from Gemini CLI, skills/subagents system, multi-provider, OAuth with 1K free daily requests", type: "open-source", url: "https://github.com/QwenLM/qwen-code", github: "https://github.com/QwenLM/qwen-code", directLayers: ["integrations"], indirectLayers: ["harness"], yc: null, subcategories: ["code-dev-tools"] },
    { id: "minimax", name: "MiniMax", desc: "Open-weight MoE models (M1, M2, M2.5) with linear attention, multimodal API platform (image/video/TTS/music generation)", type: "open-weight", url: "https://minimax.io", github: "https://github.com/MiniMax-AI", directLayers: ["pretrain", "inference"], indirectLayers: [], yc: null, subcategories: ["foundation-models", "multimodal"] },
    ...
    { id: "deeptune", name: "Deeptune", desc: "Training gyms for AI agents — high-fidelity RL environments simulating workplace software (Slack, Salesforce, ticketing, finance), sells to frontier labs for agentic RL training", type: "managed", url: "https://deeptune.com", github: null, directLayers: ["posttrain"], indirectLayers: ["eval"], yc: null, subcategories: ["alignment"]},
    ...
    { id: "omlx", name: "oMLX", desc: "macOS-native MLX inference server with paged SSD KV caching (never evicts), continuous batching (4.14x at 8x concurrency), multi-model LRU, and <5s TTFT from cache. OpenAI-compatible API. 10K+ stars", type: "open-source", url: "https://omlx.ai", github: "https://github.com/jundot/omlx", directLayers: ["inference"], indirectLayers: ["harness"], yc: null, subcategories: ["serving-engines", "local-runners", "optimization"]},
    { id: "dflash-mlx", name: "DFlash MLX", desc: "Speculative decoding on Apple Silicon — 85 tok/s, 3.3x speedup, 16-token parallel draft with single-pass verify, built on MLX. Reuses active model hidden states so adapters work with post-trained models", type: "open-source", url: "https://github.com/aryagm01/dflash-mlx", github: "https://github.com/aryagm01/dflash-mlx", directLayers: ["inference"], indirectLayers: [], yc: null, subcategories: ["local-runners", "optimization"]},
    { id: "ddtree", name: "DDTree-MLX", desc: "Tree-based speculative decoding for Apple Silicon — drafts a tree with one block diffusion pass, verifies multiple likely continuations together. Next evolution beyond linear speculative decoding", type: "open-source", url: "https://github.com/runsonai/ddtree-mlx", github: "https://github.com/runsonai/ddtree-mlx", directLayers: ["inference"], indirectLayers: [], yc: null, subcategories: ["optimization", "local-runners"]},
    { id: "pi-autoresearch", name: "pi-autoresearch (Shopify)", desc: "Open-source autoresearch harness — automated research loop with benchmark-driven optimization, shipped by Shopify Engineering", type: "open-source", url: null, github: "https://github.com/Shopify/pi-autoresearch", directLayers: ["harness", "eval"], indirectLayers: ["orchestration"], yc: null, subcategories: ["verification-loops", "evaluation"]},
    ...
    { id: "flash-moe", name: "flash-moe", desc: "SSD-streaming inference for MoE models — runs Qwen3.5-397B on 48GB MacBook at 4.36 tok/s (4-bit) via serial GPU→SSD→GPU pipeline, FMA-optimized Metal kernels, OS page cache over custom caching (58 failed experiments proved the OS wins)", type: "open-source", url: "https://github.com/danveloper/flash-moe", github: "https://github.com/danveloper/flash-moe", directLayers: ["inference"], indirectLayers: [], yc: null, subcategories: ["local-runners"]},
    { id: "llmfit", name: "llmfit", desc: "One command to find what LLMs run on your hardware — scans hundreds of models/providers, recommends optimal fit for local machine specs", type: "open-source", url: "https://github.com/AlexsJones/llmfit", github: "https://github.com/AlexsJones/llmfit", directLayers: ["inference"], indirectLayers: [], yc: null, subcategories: ["local-runners"]},
    { id: "llamafile", name: "LlamaFile", desc: "Mozilla — LLM as single cross-platform executable, zero dependencies via cosmopolitan libc", type: "open-source", url: "https://github.com/Mozilla-Ocho/llamafile", github: "https://github.com/Mozilla-Ocho/llamafile", directLayers: ["inference"], indirectLayers: [], yc: null, subcategories: ["local-runners"]},
    { id: "lobe-chat", name: "Lobe Chat", desc: "Open-source chat UI with plugin ecosystem, multi-model support, file upload, TTS, and knowledge base", type: "open-source", url: "https://chat-preview.lobehub.com", github: "https://github.com/lobehub/lobe-chat", directLayers: ["products"], indirectLayers: ["integrations"], yc: null, subcategories: ["assistants-copilots"]},
    { id: "tabby", name: "Tabby", desc: "Self-hosted AI code completion — open-source Copilot alternative with local inference", type: "open-source", url: "https://tabby.tabbyml.com", github: "https://github.com/TabbyML/tabby", directLayers: ["integrations"], indirectLayers: ["inference"], yc: null, subcategories: ["code-dev-tools"]},
    { id: "swe-agent", name: "SWE-agent", desc: "Princeton's agent-computer interface for autonomous GitHub issue resolution — pioneered ACI design patterns (capped search, stateful viewer, lint-on-edit)", type: "open-source", url: "https://swe-agent.com", github: "https://github.com/princeton-nlp/SWE-agent", directLayers: ["harness", "integrations"], indirectLayers: ["eval"], yc: null, subcategories: ["verification-loops", "code-dev-tools"]},
    { id: "letta", name: "Letta (MemGPT)", desc: "Long-term memory management for LLM agents — stateful agents with self-editing memory and persistent context across sessions", type: "open-source", url: "https://www.letta.com", github: "https://github.com/letta-ai/letta", directLayers: ["context"], indirectLayers: ["harness"], yc: null, subcategories: ["memory-systems"]},
    { id: "open-webui", name: "Open WebUI", desc: "Self-hosted ChatGPT-like interface supporting Ollama, OpenAI, and other backends", type: "open-source", url: "https://openwebui.com", github: "https://github.com/open-webui/open-webui", directLayers: ["products"], indirectLayers: ["inference"], yc: null, subcategories: ["assistants-copilots"]},
    { id: "qvac", name: "QVAC Fabric", desc: "Tether's cross-platform BitNet LoRA framework — 1-bit LLM training and inference on consumer GPUs and smartphones (13B on iPhone)", type: "open-source", url: "https://qvac.ai", github: null, directLayers: ["inference", "posttrain"], indirectLayers: ["compute"], yc: null, subcategories: ["local-runners", "efficient-adaptation"]},
    ...
    { id: "turboquant", name: "TurboQuant", desc: "Google Research — data-oblivious KV cache quantization to 3 bits with zero accuracy loss, 6x memory reduction, 8x attention speedup on H100. Two-stage: PolarQuant (random rotation → polar coordinates → Lloyd-Max scalar quantization) + QJL (1-bit residual correction for unbiased inner products). ICLR 2026", type: "open-source", url: "https://research.google/blog/turboquant-redefining-ai-efficiency-with-extreme-compression/", github: "https://github.com/amirzandieh/QJL", directLayers: ["inference"], indirectLayers: ["context"], yc: null, subcategories: ["optimization", "serving-engines"]},
    { id: "rotorquant", name: "RotorQuant", desc: "Upgraded TurboQuant — 10x KV cache compression for inference optimization", type: "open-source", url: null, github: null, directLayers: ["inference"], indirectLayers: [], yc: null, subcategories: ["optimization"]},
    { id: "lpm-1", name: "LPM 1.0", desc: "17B DiT for real-time full-duplex conversational video generation — avatar and digital human applications", type: "open-source", url: null, github: null, directLayers: ["inference"], indirectLayers: ["products"], yc: null, subcategories: ["multimodal"]},
    { id: "mlx-tune", name: "mlx-tune", desc: "Fine-tune full Gemma 4 family locally on Mac — E2B, E4B, 26B MoE, 31B Dense for text/vision/audio with unified FastVisionModel API", type: "open-source", url: "https://github.com/ARahim3/mlx-tune", github: "https://github.com/ARahim3/mlx-tune", directLayers: ["posttrain", "inference"], indirectLayers: [], yc: null, subcategories: ["efficient-adaptation", "local-runners"]},
    ...
    { id: "insanely-fast-whisper", name: "Insanely Fast Whisper", desc: "Optimized Whisper CLI — batched chunking + fp16 + Flash Attention 2 yields 19x speedup over vanilla", type: "open-source", url: "https://github.com/Vaibhavs10/insanely-fast-whisper", github: "https://github.com/Vaibhavs10/insanely-fast-whisper", directLayers: ["inference"], indirectLayers: [], yc: null, subcategories: ["multimodal", "optimization"]},
    ...
    { id: "open-swe", name: "Open SWE", desc: "Async coding agent framework — sandbox-as-backend protocol, deterministic middleware hooks (@before_model/@after_agent), thread-to-sandbox persistence, multi-surface invocation (Slack/GitHub/Linear)", type: "open-source", url: "https://github.com/langchain-ai/open-swe", github: "https://github.com/langchain-ai/open-swe", directLayers: ["harness", "orchestration"], indirectLayers: ["integrations"], yc: null, subcategories: ["execution-environments", "agent-patterns", "connectors"]},
    ...
    { id: "claude-code", name: "Claude Code", desc: "Anthropic's agentic coding tool with tool use, MCP, plugin ecosystem, file system access", type: "proprietary", url: "https://docs.anthropic.com/en/docs/agents", github: null, directLayers: ["harness", "orchestration", "integrations"], indirectLayers: ["inference", "context", "eval"], yc: null, layerDetails: { harness: "Agent runtime with sandboxed execution, tool infrastructure, context compaction, verification loops, and extensible plugin ecosystem (skills, commands, agents, MCP servers)", orchestration: "Agent loop coordinates multi-step coding tasks with planning and execution", integrations: "MCP protocol, file system access, terminal integration, git, plugin marketplace", inference: "Routes to Claude API for completions", context: "Reads project files, CLAUDE.md, maintains conversation context", eval: "Built-in cost tracking, output validation" }, subcategories: ["agent-patterns", "tool-infrastructure", "verification-loops", "context-management"]},
    { id: "openai-codex", name: "OpenAI Codex", desc: "Cloud coding agent with sandboxed execution", type: "proprietary", url: "https://openai.com/index/introducing-codex/", github: null, directLayers: ["harness", "orchestration", "integrations"], indirectLayers: ["inference", "context"], yc: null, layerDetails: { harness: "Sandboxed execution environment with AGENTS.md context, layered architectural constraints, and entropy management", orchestration: "Cloud-based agent with sandboxed multi-step execution", integrations: "File system, terminal, git integration in sandbox", inference: "Uses OpenAI models for code generation", context: "Reads repository files, maintains task context" }, subcategories: ["agent-patterns", "execution-environments", "constraints", "context-management"]},
    { id: "devin", name: "Devin (Cognition)", desc: "Autonomous software engineering agent", type: "proprietary", url: "https://www.cognition.ai", github: null, directLayers: ["harness", "orchestration", "integrations"], indirectLayers: [], yc: null, subcategories: ["agent-patterns", "execution-environments", "verification-loops"]},
    { id: "ecc", name: "Everything Claude Code", desc: "Harness optimization system — instinct-based continuous learning (observe→instinct→skill→agent evolution pipeline with project scoping), config protection against quality gate gaming, de-sloppify pattern, 119+ skills, 28 agents, cross-harness portability (Claude Code/Cursor/OpenCode/Codex/Kiro)", type: "open-source", url: "https://github.com/affaan-m/everything-claude-code", github: "https://github.com/affaan-m/everything-claude-code", directLayers: ["harness"], indirectLayers: ["orchestration", "eval"], yc: null, subcategories: ["constraints", "verification-loops", "state-continuity", "context-management"]},
    { id: "openspace", name: "OpenSpace", desc: "Self-evolving agent skill engine — three-trigger evolution (post-analysis, tool degradation, metric monitoring), three modes (FIX/DERIVED/CAPTURED), cloud skill community for collective intelligence, 54% token savings via skill reuse, MCP server integration", type: "open-source", url: "https://github.com/HKUDS/OpenSpace", github: "https://github.com/HKUDS/OpenSpace", directLayers: ["harness", "orchestration"], indirectLayers: ["eval", "context"], yc: null, subcategories: ["state-continuity", "verification-loops", "tool-infrastructure", "workflow-design"]},
    { id: "superpowers", name: "Superpowers", desc: "Agentic skills framework enforcing structured development — design, plan, execute with subagents, and mandatory TDD — across Claude Code, Cursor, Codex, and Gemini CLI", type: "open-source", url: "https://github.com/obra/superpowers", github: "https://github.com/obra/superpowers", directLayers: ["harness", "orchestration"], indirectLayers: [], yc: null, layerDetails: { harness: "Enforces test-driven development (RED-GREEN-REFACTOR), two-stage code review, systematic four-phase debugging, and git worktree isolation for parallel execution", orchestration: "Structured pipeline (Design → Plan → Execute → QA) with subagent dispatch for bite-sized tasks and Socratic design refinement" }, subcategories: ["constraints", "verification-loops", "execution-environments", "workflow-design"]},
    { id: "pua", name: "PUA", desc: "AI agent skill plugin that enforces persistence, proactive debugging, and mandatory verification across Claude Code, Codex, Cursor, and other tools", type: "open-source", url: "https://github.com/tanweai/pua", github: "https://github.com/tanweai/pua", directLayers: ["harness"], indirectLayers: [], yc: null, subcategories: ["constraints", "verification-loops"]},
    { id: "meta-harness", name: "Meta-Harness (Stanford)", desc: "Automated harness engineering — coding agent loop that optimizes the code wrapping a model using full execution traces (10M tokens), achieving +7.7 points and 4x fewer tokens on TerminalBench-2", type: "open-source", url: "https://arxiv.org/abs/2504.meta-harness", github: null, directLayers: ["harness"], indirectLayers: ["eval"], yc: null, subcategories: ["verification-loops", "context-management"]},
    { id: "citadel", name: "Citadel", desc: "Agent orchestration harness for Claude Code — cost-based tier routing (Skill→Marshal→Archon→Fleet), discovery relay between parallel agent waves, campaign persistence, lifecycle hooks as safety net, file-based coordination claims", type: "open-source", url: "https://github.com/SethGammon/Citadel", github: "https://github.com/SethGammon/Citadel", directLayers: ["harness", "orchestration"], indirectLayers: [], yc: null, subcategories: ["workflow-design", "multi-agent", "constraints", "state-continuity"]},
    { id: "gstack", name: "gstack", desc: "Software factory by YC CEO — 28 sprint-structured skills (Think→Plan→Build→Review→Test→Ship→Reflect), autoplan with encoded decision heuristics (mechanical vs taste), multi-model adversarial review (Claude+Codex consensus), persistent browser daemon with a11y-tree element refs", type: "open-source", url: "https://github.com/garrytan/gstack", github: "https://github.com/garrytan/gstack", directLayers: ["harness", "orchestration"], indirectLayers: ["integrations", "eval"], yc: null, subcategories: ["constraints", "verification-loops", "tool-infrastructure", "workflow-design"]},
    { id: "claude-hud", name: "Claude HUD", desc: "Real-time heads-up display for Claude Code — context window consumption, tool activity, subagent status, rate limits, task progress via statusLine API and transcript parsing", type: "open-source", url: "https://github.com/jarrodwatts/claude-hud", github: "https://github.com/jarrodwatts/claude-hud", directLayers: ["harness", "eval"], indirectLayers: [], yc: null, subcategories: ["context-management", "observability"]},
    { id: "gsd-v1", name: "GSD (v1)", desc: "Viral prompt framework for Claude Code — markdown slash commands for structured project execution", type: "open-source", url: "https://github.com/glittercowboy/get-shit-done", github: "https://github.com/glittercowboy/get-shit-done", directLayers: ["harness", "orchestration", "context"], indirectLayers: ["integrations"], yc: null, layerDetails: { harness: "Configures Claude Code's harness with workflow prompts and project state templates — a harness configuration layer", orchestration: "Injects workflow prompts (discuss, plan, execute, review) via Claude Code slash commands to impose project structure", context: "Prompt templates carry project state (roadmaps, task plans, decisions) into the LLM context window", integrations: "Relies entirely on Claude Code's tools (file system, terminal, git) — adds no new tool surface" }, subcategories: ["workflow-design", "context-management"]},
    { id: "gsd-2", name: "GSD 2", desc: "Spec-driven autonomous coding agent — state machine with fresh context per task, crash recovery, cost tracking, and git automation", type: "open-source", url: "https://github.com/gsd-build/GSD-2", github: "https://github.com/gsd-build/GSD-2", directLayers: ["harness", "orchestration", "context", "integrations", "eval"], indirectLayers: ["inference", "routing"], yc: null, layerDetails: { harness: "State machine with fresh 200k-token sessions per task, crash recovery, stuck detection, and file-driven progress tracking — a complete agent runtime harness", orchestration: "State machine decomposes work into Milestone → Slice → Task, dispatches each in a fresh 200k-token session, handles stuck detection, timeout supervision, and adaptive replanning", context: "Pre-inlines task plans, prior summaries, decisions register, and dependency context into each dispatch prompt — LLM never wastes tool calls on orientation", integrations: "Programmatic git branch-per-slice with squash merge, subagent spawning (Scout, Researcher, Worker), Playwright browser, Brave Search, Context7 docs", eval: "Per-unit cost/token ledger with phase and model breakdowns, budget ceiling enforcement, observability validators at phase boundaries, must-have verification", inference: "Routes to 20+ LLM providers via Pi SDK — supports per-phase model selection (Opus for planning, Sonnet for execution)", routing: "Multi-model preferences allow different models per workflow phase, with automatic model switching at dispatch time" }, subcategories: ["agent-patterns", "workflow-design", "state-continuity", "verification-loops", "context-management"]},
    ...
    { id: "metagpt", name: "MetaGPT", desc: "Multi-agent framework where agents play software company roles (PM, architect, engineer, QA) with SOPs and structured outputs", type: "open-source", url: "https://github.com/geekan/MetaGPT", github: "https://github.com/geekan/MetaGPT", directLayers: ["orchestration"], indirectLayers: [], yc: null, subcategories: ["multi-agent"]},
    { id: "flowise", name: "Flowise", desc: "Drag-and-drop LLM flow builder with visual node editor for chains, agents, and RAG pipelines", type: "open-source", url: "https://flowiseai.com", github: "https://github.com/FlowiseAI/Flowise", directLayers: ["orchestration"], indirectLayers: [], yc: null, subcategories: ["workflow-design"]},
    { id: "langflow", name: "Langflow", desc: "Visual drag-and-drop builder for AI agent pipelines and workflows", type: "open-source", url: "https://www.langflow.org", github: "https://github.com/langflow-ai/langflow", directLayers: ["orchestration"], indirectLayers: [], yc: null, subcategories: ["workflow-design"]},
    ...
    { id: "agent-orchestrator", name: "Agent Orchestrator", desc: "Composio's agent fleet manager — runs 30+ coding agents in parallel with plugin architecture (8 slots, 20+ plugins), closed-loop CI/review feedback routing, LLM-driven task decomposition, git worktree isolation", type: "open-source", url: "https://github.com/ComposioHQ/agent-orchestrator", github: "https://github.com/ComposioHQ/agent-orchestrator", directLayers: ["orchestration"], indirectLayers: ["harness"], yc: "W24", subcategories: ["multi-agent"]},
    { id: "warp-oz", name: "Warp Oz", desc: "Enterprise cloud agent orchestration platform — harness-agnostic control plane with policies, tracking, handoff, sandboxing, memory layer, and self-hosting support", type: "proprietary", url: "https://www.warp.dev", github: null, directLayers: ["orchestration"], indirectLayers: ["harness", "eval"], yc: null, subcategories: ["agent-patterns", "workflow-design", "multi-agent"]},
    { id: "opencli", name: "OpenCLI", desc: "Universal CLI over any website via browser session parasitism — reuses Chrome's authenticated sessions, auto-discovers APIs via explore/synthesize/cascade, LLM-optimized DOM snapshots with action-indexed elements", type: "open-source", url: "https://github.com/jackwener/opencli", github: "https://github.com/jackwener/opencli", directLayers: ["integrations"], indirectLayers: ["harness"], yc: null, subcategories: ["connectors"]},
    { id: "opencli-rs", name: "opencli-rs", desc: "Rust rewrite of OpenCLI — 4.7MB static binary, 55+ sites, 333 commands, 5-12x faster, pest PEG parser for safe templates, declarative YAML pipeline adapters with 14 step types, zero runtime deps", type: "open-source", url: "https://github.com/nashsu/opencli-rs", github: "https://github.com/nashsu/opencli-rs", directLayers: ["integrations"], indirectLayers: ["harness"], yc: null, subcategories: ["connectors"]},
    { id: "clearshot", name: "Clearshot", desc: "Structured screenshot analysis skill for Claude Code — extracts hex colors, exact spacing, component specs from UI screenshots via three-level analysis (Map/System/Blueprint), auto-triggers on UI images", type: "open-source", url: "https://github.com/udayanwalvekar/clearshot", github: "https://github.com/udayanwalvekar/clearshot", directLayers: ["harness"], indirectLayers: ["integrations"], yc: null, subcategories: ["tool-infrastructure"]},
    { id: "aurl", name: "aurl", desc: "API spec to CLI — projects OpenAPI/GraphQL specs into named subcommands with auto-generated help, spec-driven validation, and auth management for agent-first API consumption", type: "open-source", url: "https://github.com/ShawnPana/aurl", github: "https://github.com/ShawnPana/aurl", directLayers: ["integrations"], indirectLayers: [], yc: null, subcategories: ["connectors"]},
    { id: "e2b", name: "E2B", desc: "Sandboxed code execution for agents", type: "managed", url: "https://e2b.dev", github: "https://github.com/e2b-dev/e2b", directLayers: ["harness"], indirectLayers: [], yc: "W24", subcategories: ["execution-environments"]},
    { id: "agent-browser", name: "agent-browser", desc: "Rust CLI for AI browser automation — accessibility-tree snapshots, semantic locators, batch commands, daemon sessions", type: "open-source", url: "https://github.com/vercel-labs/agent-browser", github: "https://github.com/vercel-labs/agent-browser", directLayers: ["harness"], indirectLayers: ["orchestration", "integrations"], yc: null, subcategories: ["tool-infrastructure", "execution-environments"]},
    ...
    { id: "hermes-agent", name: "Hermes Agent", desc: "Self-improving AI agent with persistent memory, autonomous skill creation, multi-platform messaging, and subagent delegation", type: "open-source", url: "https://github.com/NousResearch/hermes-agent", github: "https://github.com/NousResearch/hermes-agent", directLayers: ["harness", "orchestration", "context", "integrations"], indirectLayers: ["inference", "routing"], yc: null, layerDetails: { harness: "Complete agent runtime with multiple execution backends (Docker, SSH, Daytona, Modal), tool infrastructure via MCP, and self-improving skill loops", orchestration: "Subagent delegation with isolated parallel workstreams and cron-scheduled task automation", context: "Procedural memory system with FTS5 search, LLM summarization, and persistent cross-session knowledge", integrations: "Unified messaging gateway (Telegram, Discord, Slack, WhatsApp, Signal) and MCP server support" }, subcategories: ["agent-patterns", "execution-environments", "state-continuity", "verification-loops"]},
    ...
    { id: "dexter", name: "Dexter", desc: "Autonomous agent for deep financial research — team of agents analyzing stocks like world-class investors, 50K GitHub stars", type: "open-source", url: "https://github.com/virattt/dexter", github: "https://github.com/virattt/dexter", directLayers: ["orchestration", "products"], indirectLayers: [], yc: null, subcategories: ["agent-patterns", "autonomous-agents"]},
    ...
    { id: "opendataloader-pdf", name: "OpenDataLoader PDF", desc: "Hybrid PDF parser for RAG — triage routes simple pages to deterministic Java core (0.05s/page), complex pages to AI backend; per-element bounding boxes for citation, prompt injection defense via hidden text detection", type: "open-source", url: "https://github.com/opendataloader-project/opendataloader-pdf", github: "https://github.com/opendataloader-project/opendataloader-pdf", directLayers: ["context"], indirectLayers: ["eval"], yc: null, subcategories: ["retrieval-search"]},
    ...
    { id: "a2ui", name: "A2UI", desc: "Google's Agent-to-User Interface protocol — declarative JSON format for agents to generate rich interactive UIs without executing code", type: "open-source", url: "https://github.com/google/A2UI", github: "https://github.com/google/A2UI", directLayers: ["integrations"], indirectLayers: ["products"], yc: null, subcategories: ["protocols-standards"]},
    ...
    { id: "copilotkit", name: "CopilotKit", desc: "Frontend framework for embedding AI copilots and generative UI into React apps, creators of the AG-UI protocol", type: "open-source", url: "https://www.copilotkit.ai", github: "https://github.com/CopilotKit/CopilotKit", directLayers: ["integrations"], indirectLayers: ["products"], yc: null, subcategories: ["connectors"]},
    { id: "json-render", name: "json-render", desc: "Vercel Generative UI framework — LLMs output catalog-constrained JSON specs rendered across 10 platforms (React, Vue, Svelte, Native, PDF, video, 3D) with streaming and stateful prop expressions", type: "open-source", url: "https://json-render.dev", github: "https://github.com/vercel-labs/json-render", directLayers: ["integrations"], indirectLayers: ["products"], yc: null, subcategories: ["protocols-standards"]},
    { id: "codexbar", name: "CodexBar", desc: "macOS menu bar app showing real-time AI coding tool quotas — tracks 16 providers (Codex, Claude, Cursor, Gemini, Copilot), session/weekly limits, reset countdowns", type: "open-source", url: "https://github.com/steipete/CodexBar", github: "https://github.com/steipete/CodexBar", directLayers: ["integrations"], indirectLayers: ["eval"], yc: null, subcategories: ["code-dev-tools"]},
    { id: "claude-squad", name: "claude-squad", desc: "Terminal multiplexer for running multiple AI coding agents in parallel tmux sessions with shared worktree isolation", type: "open-source", url: "https://github.com/smtg-ai/claude-squad", github: "https://github.com/smtg-ai/claude-squad", directLayers: ["orchestration"], indirectLayers: ["harness"], yc: null, subcategories: ["multi-agent"]},
    { id: "collaborator", name: "Collaborator", desc: "Infinite canvas workspace for agentic dev — terminals, code, notes as spatial tiles with tmux-backed persistent agent sessions", type: "open-source", url: "https://github.com/collaborator-ai/collab-public", github: "https://github.com/collaborator-ai/collab-public", directLayers: ["integrations"], indirectLayers: ["harness"], yc: null, subcategories: ["code-dev-tools"]},
    ...
    { id: "pentagi", name: "PentAGI", desc: "Autonomous pentesting agent — ChainAST for conversations (typed AST with structural validation, byte tracking, force-repair), Generator/Refiner adaptive replanning, 12+ specialized agents, three-phase summarization with provider-specific reasoning preservation", type: "open-source", url: "https://github.com/vxcontrol/pentagi", github: "https://github.com/vxcontrol/pentagi", directLayers: ["harness", "orchestration"], indirectLayers: ["eval", "context"], yc: null, subcategories: ["context-management", "agent-patterns", "workflow-design"]},
    ...
    { id: "autensa", name: "Autensa", desc: "Open-source AI agent orchestration dashboard — task management, AI planning, multi-agent dispatch via OpenClaw Gateway", type: "open-source", url: "https://autensa.com", github: "https://github.com/crshdn/mission-control", directLayers: ["orchestration"], indirectLayers: [], yc: null, subcategories: ["agent-patterns", "human-oversight"]},
    ...
    { id: "llm-arch-gallery", name: "LLM Architecture Gallery", desc: "Visual catalog of 43+ modern LLM architectures with specs, diagrams, and pattern analysis by Sebastian Raschka", type: "open-source", url: "https://sebastianraschka.com/llm-architecture-gallery/", github: "https://github.com/rasbt/llm-architecture-gallery", directLayers: ["pretrain"], indirectLayers: [], yc: null, subcategories: ["foundation-models"]},
    ...
    { id: "bifrost", name: "Bifrost", desc: "High-performance open-source AI gateway in Go — 15+ providers, semantic caching, MCP support, ~11us overhead", type: "open-source", url: "https://github.com/maximhq/bifrost", github: "https://github.com/maximhq/bifrost", directLayers: ["routing"], indirectLayers: ["inference"], yc: null, subcategories: ["gateways-proxies"]},

    // ── New: Harness / Orchestration ──
    { id: "learn-claude-code", name: "Learn Claude Code", desc: "12-session progressive guide to building AI coding agents — from basic loop to multi-agent worktree isolation", type: "open-source", url: "https://github.com/shareAI-lab/learn-claude-code", github: "https://github.com/shareAI-lab/learn-claude-code", directLayers: ["harness", "orchestration"], indirectLayers: ["context"], yc: null, subcategories: ["execution-environments", "agent-patterns"]},
    { id: "opencode", name: "OpenCode", desc: "Open-source terminal AI coding agent — provider-agnostic, LSP integration, client/server architecture", type: "open-source", url: "https://github.com/anomalyco/opencode", github: "https://github.com/anomalyco/opencode", directLayers: ["integrations", "products"], indirectLayers: ["harness"], yc: null, subcategories: ["code-dev-tools", "assistants-copilots"]},
    { id: "claudian", name: "Claudian", desc: "Obsidian plugin integrating Claude Code as an agentic vault collaborator — file ops, vision, MCP, slash commands", type: "open-source", url: "https://github.com/YishenTu/claudian", github: "https://github.com/YishenTu/claudian", directLayers: ["integrations", "products"], indirectLayers: ["context"], yc: null, subcategories: ["code-dev-tools", "assistants-copilots"]},
    { id: "superpowers", name: "Superpowers", desc: "Composable skills framework (14 skills, 6 platforms) — enforces brainstorm→plan→TDD pipeline with subagent-driven development, two-stage review, and hard-gate verification", type: "open-source", url: "https://github.com/obra/superpowers", github: "https://github.com/obra/superpowers", directLayers: ["harness", "orchestration"], indirectLayers: [], yc: null, subcategories: ["verification-loops", "constraints", "workflow-design", "multi-agent"]},
    { id: "mirofish", name: "MiroFish", desc: "Swarm intelligence prediction engine — simulates thousands of autonomous agents to forecast outcomes and test decisions", type: "open-source", url: "https://github.com/666ghj/MiroFish", github: "https://github.com/666ghj/MiroFish", directLayers: ["orchestration"], indirectLayers: ["products"], yc: null, subcategories: ["multi-agent"]},
    { id: "clawwork", name: "ClawWork", desc: "OpenClaw as autonomous AI coworker — earned $15K in 11 hours, from HKU Data Science lab", type: "open-source", url: "https://github.com/HKUDS/ClawWork", github: "https://github.com/HKUDS/ClawWork", directLayers: ["orchestration", "products"], indirectLayers: ["harness"], yc: null, subcategories: ["autonomous-agents"]},
    { id: "clawteam", name: "ClawTeam", desc: "Framework-agnostic multi-agent CLI — leader agents spawn workers, coordinate via filesystem messaging, isolate work in git worktrees", type: "open-source", url: "https://github.com/HKUDS/ClawTeam", github: "https://github.com/HKUDS/ClawTeam", directLayers: ["orchestration"], indirectLayers: ["harness"], yc: null, subcategories: ["multi-agent"]},
    { id: "google-adk", name: "Google ADK", desc: "Open-source code-first agent framework — multi-agent composition, MCP tools, built-in eval, multi-language", type: "open-source", url: "https://google.github.io/adk-docs/", github: "https://github.com/google/adk-python", directLayers: ["orchestration"], indirectLayers: [], yc: null, subcategories: ["agent-patterns"]},
    ...
    { id: "dedalus-labs", name: "Dedalus Labs", desc: "Hosts MCP servers with autoscaling and one-click deployment", type: "managed", url: "https://dedaluslabs.com", github: null, directLayers: ["harness", "integrations"], indirectLayers: [], yc: "S25", subcategories: ["execution-environments"]},
    ...
    { id: "castari", name: "Castari", desc: "Deploy AI agents in secure, autoscaling sandboxes with MCP connectors", type: "managed", url: "https://castari.com", github: null, directLayers: ["harness"], indirectLayers: ["integrations"], yc: "S25", subcategories: ["execution-environments"]},
    { id: "kernel-ai", name: "Kernel (AI infra)", desc: "Open-source unikernel infra for AI agents to access the internet with sub-150ms cold starts", type: "open-source", url: "https://github.com/kernelai", github: null, directLayers: ["harness"], indirectLayers: ["integrations"], yc: "S25", subcategories: ["execution-environments"]},
    { id: "nemoclaw", name: "NemoClaw", desc: "NVIDIA agent sandbox — binary-scoped network policies (per-binary, per-endpoint, per-HTTP-method), inference interception via gateway, Landlock/seccomp/netns enforcement, immutable config with operator-in-the-loop approval", type: "open-source", url: "https://github.com/NVIDIA/NemoClaw", github: "https://github.com/NVIDIA/NemoClaw", directLayers: ["harness", "eval"], indirectLayers: ["routing"], yc: null, subcategories: ["execution-environments", "guardrails-security"]},
    { id: "make", name: "Make", desc: "Visual automation platform with AI agent capabilities and 1,800+ app connectors", type: "managed", url: "https://www.make.com", github: null, directLayers: ["orchestration", "integrations"], indirectLayers: [], yc: null, subcategories: ["workflow-design", "connectors"]},
    { id: "uipath", name: "UiPath", desc: "Enterprise RPA platform adding agentic automation with AI reasoning on top of process automation", type: "managed", url: "https://www.uipath.com", github: null, directLayers: ["orchestration", "integrations"], indirectLayers: [], yc: null, subcategories: ["workflow-design", "connectors"]},
    { id: "openai-frontier", name: "OpenAI Frontier", desc: "Enterprise platform for building and managing AI agents with integrated governance, permissions, and multi-provider support", type: "managed", url: "https://openai.com", github: null, directLayers: ["orchestration", "products"], indirectLayers: ["harness", "integrations", "eval"], yc: null, subcategories: ["agent-patterns", "multi-agent"]},

    // ── New: Harness / Products ──
    { id: "deerflow", name: "DeerFlow", desc: "ByteDance super agent harness — LangGraph runtime with Docker sandbox, ordered middleware chain (9 middlewares for context/memory/uploads/clarification), progressive skill loading, sub-agent spawning, IM channel gateway (Telegram/Slack/Feishu)", type: "open-source", url: "https://github.com/bytedance/deer-flow", github: "https://github.com/bytedance/deer-flow", directLayers: ["harness", "orchestration"], indirectLayers: ["context", "integrations"], yc: null, subcategories: ["execution-environments", "agent-patterns"]},
    { id: "goose", name: "Goose", desc: "Open-source extensible AI agent by Block — Rust-based, multi-model, 1700+ MCP servers, desktop + CLI", type: "open-source", url: "https://github.com/block/goose", github: "https://github.com/block/goose", directLayers: ["harness", "integrations"], indirectLayers: ["orchestration"], yc: null, subcategories: ["execution-environments", "code-dev-tools"]},
    { id: "gemini-cli", name: "Gemini CLI", desc: "Google's open-source terminal AI agent — Gemini models, 1M context, MCP support, Google Search grounding", type: "open-source", url: "https://github.com/google-gemini/gemini-cli", github: "https://github.com/google-gemini/gemini-cli", directLayers: ["integrations", "products"], indirectLayers: [], yc: null, subcategories: ["code-dev-tools", "assistants-copilots"]},

    // ── New: Context ──
    { id: "openviking", name: "OpenViking", desc: "ByteDance's context database for AI agents — filesystem paradigm with three-tier demand-based loading, recursive retrieval, and observable retrieval trajectories", type: "open-source", url: "https://github.com/volcengine/OpenViking", github: "https://github.com/volcengine/OpenViking", directLayers: ["context"], indirectLayers: ["harness"], yc: null, subcategories: ["memory-systems", "retrieval-search"]},
    ...
    { id: "context-hub", name: "Context Hub", desc: "Andrew Ng's CLI for curated, versioned API docs for coding agents — self-improving loop with agent annotations and author feedback", type: "open-source", url: "https://github.com/andrewyng/context-hub", github: "https://github.com/andrewyng/context-hub", directLayers: ["context"], indirectLayers: ["integrations"], yc: null, subcategories: ["retrieval-search"]},
    ...
    { id: "ragflow", name: "RAGFlow", desc: "Open-source RAG engine with deep document parsing, intelligent chunking, and citation grounding", type: "open-source", url: "https://ragflow.io", github: "https://github.com/infiniflow/ragflow", directLayers: ["context"], indirectLayers: [], yc: null, subcategories: ["retrieval-search"]},
    { id: "claude-mem", name: "Claude-Mem", desc: "Persistent memory plugin for Claude Code — auto-captures session context, AI-compressed retrieval, progressive disclosure", type: "open-source", url: "https://github.com/thedotmack/claude-mem", github: "https://github.com/thedotmack/claude-mem", directLayers: ["context"], indirectLayers: ["harness"], yc: null, subcategories: ["memory-systems"]},
    { id: "hindsight", name: "Hindsight", desc: "Biomimetic agent memory — world facts, experiences, and mental models with retain/recall/reflect operations", type: "open-source", url: "https://hindsight.vectorize.io", github: "https://github.com/vectorize-io/hindsight", directLayers: ["context"], indirectLayers: ["harness"], yc: null, subcategories: ["memory-systems"]},
    ...
    { id: "iron-sensor", name: "iron-sensor", desc: "eBPF kernel-level behavioral monitor for AI coding agents — detects persistence, privilege escalation, sensitive file access via syscall tracepoints", type: "open-source", url: "https://github.com/ironsh/iron-sensor", github: "https://github.com/ironsh/iron-sensor", directLayers: ["eval"], indirectLayers: ["harness"], yc: null, subcategories: ["guardrails-security", "observability"]},
    ...
    { id: "paperbench", name: "PaperBench", desc: "OpenAI benchmark evaluating AI agents' ability to replicate research papers end-to-end (8316 gradable tasks)", type: "open-source", url: "https://openai.com/index/paperbench/", github: "https://github.com/openai/preparedness", directLayers: ["eval"], indirectLayers: [], yc: null, subcategories: ["evaluation"]},
    { id: "bloom-eval", name: "Bloom", desc: "Anthropic's automated behavioral evaluation framework — generates scenarios, quantifies frequency/severity, 0.86 human correlation", type: "open-source", url: "https://www.anthropic.com/research/bloom", github: "https://github.com/safety-research/bloom", directLayers: ["eval"], indirectLayers: [], yc: null, subcategories: ["evaluation"]},

    // ── New: Formal Verification ──
    { id: "opengauss", name: "OpenGauss", desc: "Multi-agent Lean 4 formalization orchestrator with On-Policy Distillation — converts tool feedback (compiler errors, test failures) into per-token training signal via hindsight relabeling", type: "open-source", url: "https://github.com/math-inc/OpenGauss", github: "https://github.com/math-inc/OpenGauss", directLayers: ["posttrain", "eval"], indirectLayers: ["harness", "orchestration"], yc: null, subcategories: ["alignment", "formal-verification"]},
    ...
    { id: "math-inc", name: "Math Inc", desc: "Industrial-scale autoformalization — Gauss agent synthesizes Lean proofs from research papers (200K lines for Fields Medal sphere packing proof), compounding library creates virtuous training cycle", type: "managed", url: "https://mathematics.inc", github: null, directLayers: ["eval"], indirectLayers: ["pretrain"], yc: null, subcategories: ["formal-verification"]},
    ...
    { id: "pi-autoresearch", name: "pi-autoresearch", desc: "Autonomous experiment loop for AI agents — propose, measure, retain, discard, repeat with persistent state across sessions", type: "open-source", url: "https://github.com/davebcn87/pi-autoresearch", github: "https://github.com/davebcn87/pi-autoresearch", directLayers: ["harness", "eval"], indirectLayers: [], yc: null, subcategories: ["verification-loops", "state-continuity", "performance-optimization"]},
    { id: "codex-autoresearch", name: "Codex Autoresearch", desc: "Autonomous experimentation skill for Codex CLI — graduated stuck recovery (REFINE/PIVOT/Search), protocol re-anchoring against context drift, cross-run lessons with time-decay, parallel experiments via git worktrees", type: "open-source", url: "https://github.com/leo-lilinxiao/codex-autoresearch", github: "https://github.com/leo-lilinxiao/codex-autoresearch", directLayers: ["harness", "eval"], indirectLayers: ["orchestration"], yc: null, subcategories: ["verification-loops", "state-continuity", "performance-optimization", "context-management"]},
    { id: "claude-autoresearch", name: "Claude Autoresearch", desc: "Domain-agnostic autonomous loop as a Claude Code skill — pure Markdown behavioral protocol, noise-aware metric handling (multi-run median, confirmation runs, min delta thresholds), composable command chaining across 9 specialized workflows", type: "open-source", url: "https://github.com/uditgoenka/autoresearch", github: "https://github.com/uditgoenka/autoresearch", directLayers: ["harness", "eval"], indirectLayers: [], yc: null, subcategories: ["verification-loops", "performance-optimization", "constraints"]},
    { id: "unslop", name: "Unslop", desc: "Empirical output-mode-collapse detector — generates N samples, self-audits behavioral defaults with frequency counts, produces subtractive constraint files (prohibitions, not prescriptions)", type: "open-source", url: "https://github.com/mshumer/unslop", github: "https://github.com/mshumer/unslop", directLayers: ["harness", "eval"], indirectLayers: [], yc: null, subcategories: ["constraints", "evaluation"]},

    { id: "autoresearchclaw", name: "AutoResearchClaw", desc: "Autonomous research pipeline — 23-stage state machine with ground-truth registries (anti-fabrication), multi-agent debate for hypothesis generation, self-healing experiments, recursive PIVOT re-execution, four-layer citation verification", type: "open-source", url: "https://github.com/aiming-lab/AutoResearchClaw", github: "https://github.com/aiming-lab/AutoResearchClaw", directLayers: ["orchestration", "eval"], indirectLayers: ["harness"], yc: null, subcategories: ["workflow-design", "evaluation", "verification-loops"]},
    ...
    { id: "shinka-evolve", name: "ShinkaEvolve", desc: "Multi-island evolutionary optimization framework combining LLM ensembles with evolutionary algorithms for scientific discovery", type: "open-source", url: "https://github.com/SakanaAI/ShinkaEvolve", github: "https://github.com/SakanaAI/ShinkaEvolve", directLayers: ["eval", "orchestration"], indirectLayers: [], yc: null, subcategories: ["performance-optimization", "agent-patterns"]},
    { id: "hyperagents", name: "HyperAgents", desc: "Meta AI self-referential agents — task agent + meta agent in one editable program, metacognitive self-modification where the improvement mechanism improves itself, cross-domain meta-transfer", type: "open-source", url: "https://arxiv.org/abs/2603.19461", github: "https://github.com/facebookresearch/Hyperagents", directLayers: ["eval"], indirectLayers: ["orchestration"], yc: null, subcategories: ["performance-optimization"]},
    ...
    // ── New: AI Red Teaming ──
    { id: "strix", name: "Strix", desc: "Open-source LLM-powered autonomous pentesting agent — Docker-sandboxed agent loop with browser, proxy, terminal, produces validated PoC exploits for business logic, auth bypass, injection flaws", type: "open-source", url: "https://github.com/usestrix/strix", github: "https://github.com/usestrix/strix", directLayers: ["eval"], indirectLayers: ["harness"], yc: null, subcategories: ["ai-red-teaming"]},
    { id: "codewall-ai", name: "CodeWall AI", desc: "Autonomous AI security testing agent — found 22 zero-auth endpoints and 46.5M exposed records in McKinsey's Lilli in 2 hours", type: "managed", url: "https://codewall.ai", github: null, directLayers: ["eval"], indirectLayers: [], yc: null, subcategories: ["ai-red-teaming"]},
    ...
    { id: "notebooklm", name: "NotebookLM", desc: "Google's research assistant with source-grounded citations and audio overviews", type: "proprietary", url: "https://notebooklm.google.com", github: null, directLayers: ["products"], indirectLayers: ["context"], yc: null, category: "search", subcategories: ["assistants-copilots"]},
    { id: "project-nomad", name: "Project N.O.M.A.D.", desc: "Offline-first AI knowledge server — bundles local LLM (Ollama), vector DB (Qdrant), Wikipedia/encyclopedia (Kiwix), education (Kolibri), and maps into one air-gapped appliance with ZIM-to-RAG pipeline", type: "open-source", url: "https://github.com/Crosstalk-Solutions/project-nomad", github: "https://github.com/Crosstalk-Solutions/project-nomad", directLayers: ["products"], indirectLayers: ["inference", "context"], yc: null, category: "platform", subcategories: ["assistants-copilots"]}
  ]
};
...
const VC_LAYER_IMPACT = {
  ...
  harness: "whether a single agent is effective — sandboxing, state, tools, verification, and constraints that turn an API call into a functioning agent",
  orchestration: "whether multiple agents coordinate effectively — workflows, delegation, handoffs, and human oversight",
...
const VC_LAYER_ROLE = {
  ...
  harness: "wraps a model with runtime infrastructure — sandboxes, tools, state, verification — to make it a capable agent",
  orchestration: "coordinates multiple agents, multi-step workflows, and human oversight",
...
const VC_TOOL_NARRATIVES = {
  ...
  "claude-code": {
    ...
    role: "In this map, Claude Code spans harness, orchestration, integrations, context, and inference. It is not only a model endpoint; it is a complete harness that turns a model into a functioning coding agent.",
    layerDetails: {
      harness: "Claude Code is a harness: it wraps the Claude model with tool infrastructure (file system, terminal, git, MCP), context management (CLAUDE.md, project files, conversation compaction), and verification loops (test runs, command output validation). The harness is the durable asset; the model is swappable.",
  ...
  "openai-codex": {
    what: "OpenAI Codex is a cloud coding agent that can operate over a task in a managed execution environment rather than acting as a pure autocomplete endpoint. Its 1M-line codebase was built entirely by agents — zero hand-written code, 3 engineers, 5 months — making it a canonical example of harness engineering.",
    impact: "Its impact is on task automation depth. It moves coding assistance from local suggestion generation toward remote task execution, which changes how teams think about sandboxing, review, and trust boundaries. The Codex team demonstrated that constraints multiply agent effectiveness: dynamic AGENTS.md files, dependency layering enforced by linters, and background entropy management agents.",
    role: "In this map, Codex belongs to harness, orchestration, and integrations. The harness is the core insight: sandboxed execution environments, AGENTS.md as dynamic context, architectural constraints via linters, and entropy management through background agents.",
    layerDetails: {
      harness: "Codex is the canonical harness engineering example: sandboxed execution, AGENTS.md as dynamic context, layered architectural constraints enforced by custom linters, and background entropy management agents that scan for stale docs and pattern drift.",
  ...
  "gsd-v1": {
    ...
    role: "In this map, GSD v1 sits in harness, orchestration, and context. It is a harness configuration — prompt templates that shape how Claude Code's existing harness behaves.",
    layerDetails: {
      harness: "GSD v1 is a harness configuration layer: it shapes Claude Code's agent runtime by injecting workflow prompts and project state templates. It does not build its own harness — it configures an existing one.",
  ...
  "gsd-2": {
    ...
    role: "In this map, GSD 2 spans harness, orchestration, context, integrations, and eval. It is a complete harness — a full execution engine where state management, context injection, tool integration, and quality verification are all handled deterministically in TypeScript rather than delegated to the LLM.",
    layerDetails: {
      harness: "GSD 2 is a complete agent harness: file-driven state machine for durable execution, fresh sessions per task for context management, crash recovery, stuck detection, and must-have verification loops. The harness is the entire value — the model is a swappable commodity inside it.",
  ...
  "codewall-ai": {
    what: "CodeWall AI is an autonomous AI security testing agent. It found 22 zero-auth endpoints and 46.5 million exposed records in McKinsey's Lilli chatbot in under two hours — every issue missed by commercial scanners.",
    impact: "Its impact is proof that AI-on-AI red teaming finds vulnerabilities traditional scanners cannot. AI systems create novel attack surfaces — RAG poisoning, prompt extraction, vector store exfiltration — that require iterative reasoning, not signature matching.",
    role: "In this map, CodeWall sits in the eval layer because its job is adversarial testing of AI systems before and after deployment.",
    layerDetails: {
      eval: "CodeWall uses an autonomous agent to reason iteratively about AI-specific attack surfaces: unauthenticated API endpoints, extractable system prompts, poisonable RAG pipelines, and exposed vector stores."
    }
  },
  "notebooklm": {
    what: "NotebookLM is Google's research assistant that grounds every claim in source citations. Users upload documents and get summaries, Q&A, and audio overviews where each answer links back to the specific passage that supports it.",
    impact: "Its impact is on trust in AI-generated answers. Citations are the real product — they turn summaries into auditable, traceable answers rather than opaque model outputs.",
    role: "In this map, NotebookLM sits in the products layer and touches context because its citation-grounding RAG architecture is the mechanism that makes the product work.",
    layerDetails: {
      products: "NotebookLM is an end-user research assistant — users interact with it directly to understand and synthesize their documents.",
      context: "Its citation-grounding pipeline retrieves specific passages from uploaded sources and attaches them to every generated claim, making the context layer visible to users."
    }
  },
  "codex-autoresearch": {
    what: "Codex Autoresearch turns OpenAI's Codex CLI into an autonomous researcher. A human defines a goal, a metric command, and scope; the agent then runs an indefinite modify-verify-decide loop — one atomic change per iteration, committed to git, measured, kept or rolled back. It introduces graduated stuck recovery (REFINE → PIVOT → web search → soft blocker), protocol re-anchoring against context drift, and a persistent lessons file with time-decay consolidation across runs.",
    impact: "Its impact is on long-running autonomy patterns. It solves four problems that block unattended agent execution: getting stuck (graduated escalation), forgetting instructions (protocol fingerprint checks), losing cross-session knowledge (lessons protocol), and wasting iterations on infeasible approaches (environment-aware hypothesis filtering). The two-phase interaction boundary — wizard setup then fully autonomous — makes overnight runs practical.",
    role: "In this map, Codex Autoresearch spans harness and eval with indirect ties to orchestration. The harness layer provides state continuity, context management, and verification loops; the eval layer provides the metric-driven optimization cycle; orchestration enters through parallel experiments dispatched to git worktree workers.",
    layerDetails: {
      harness: "Four harness innovations: (1) graduated stuck recovery as an escalation ladder, (2) protocol re-anchoring via fingerprint self-checks, (3) cross-run lessons with time-decay as persistent meta-learning, (4) environment probing to filter infeasible hypotheses before wasting iterations.",
      eval: "The core loop is metric-driven: one atomic change → commit → verify command → keep or rollback. A dual gate separates metric improvement from guard verification (regression tests, lint). Marginal metric gain with significant complexity increase equals discard.",
      orchestration: "Parallel experiments dispatch N hypotheses to isolated git worktrees, each with a subagent. The orchestrator selects the best result by metric improvement (ties broken by smaller diff) and cherry-picks the winner."
    }
  },
  "unslop": {
    what: "Unslop detects output-level mode collapse in LLM-generated content. It generates a large sample set (50+ outputs) across diverse prompts for a domain, feeds the entire corpus back to the LLM for self-audit, and extracts recurring behavioral patterns with exact quotes and frequency counts. The output is a skill file of specific prohibitions — subtractive constraints that tell the model what NOT to do, never what to do instead.",
    impact: "Its impact is on the quality of constraint engineering. Most attempts to fight LLM defaults use vague instructions ('be creative') or prescriptive alternatives (which just create new defaults). Unslop takes an empirical approach: measure the actual output distribution, identify statistical attractors, and prohibit them specifically. The subtractive philosophy — never prescribe replacements — is the key insight: removing defaults forces genuine variation instead of swapping one pattern for another.",
    role: "In this map, Unslop spans harness and eval. The constraint files it produces are harness-layer artifacts (06.6 Constraints & Linting) that narrow the solution space at generation time. The sample-and-analyze workflow is an eval-layer process that measures output quality empirically.",
    layerDetails: {
      harness: "Unslop produces constraint files (skill.md) that function like architectural linters but for output style rather than code structure. They're composable — drop into CLAUDE.md, Cursor rules, or any system prompt. The subtractive constraint philosophy (prohibit, never prescribe) prevents creating new defaults while eliminating old ones.",
      eval: "The generate-analyze loop is empirical evaluation: produce N samples, measure pattern frequency, identify behavioral attractors. For visual domains, Playwright renders HTML to screenshots and Claude's vision identifies perceptual patterns (layout, color, spacing defaults) invisible in source code."
    }
  },
  "nemoclaw": {
    what: "NemoClaw is NVIDIA's agent sandbox for OpenClaw. It treats autonomous AI agents as untrusted code — same philosophy as sandboxing third-party dependencies — and enforces security at four layers: Landlock LSM (filesystem), seccomp (syscalls), network namespaces (network), and DAC (file permissions). The key innovation is binary-scoped network policies: within a single container, git, npm, and the agent binary each get different endpoint and HTTP method permissions.",
    impact: "Its impact is on agent security architecture. It shifts the framing from 'sandbox the code the agent generates' to 'sandbox the agent itself.' Binary-scoped policies mean the agent cannot exfiltrate data through git or npm even if it controls those tools. Inference interception through a gateway prevents unauthorized model API calls. The immutable config / mutable state split prevents agents from modifying their own auth tokens or gateway configuration.",
    role: "In this map, NemoClaw spans harness and eval with indirect ties to routing. The harness layer provides execution environments with defense-in-depth isolation. The eval layer provides runtime security enforcement. Routing enters because inference interception through the OpenShell gateway is a security-motivated routing decision.",
    layerDetails: {
      harness: "Four-layer defense for agent execution: (1) binary-scoped network policies — per-binary, per-endpoint, per-HTTP-method access control, (2) immutable config with symlinks to mutable state — prevents agent self-modification, (3) inference interception — all model calls routed through gateway transparently, (4) operator-in-the-loop approval for unrecognized endpoints with session-scoped persistence.",
      eval: "Runtime security enforcement: deny-by-default network policy, SSRF prevention (blocks private IP ranges), audit trails for all agent actions, and composable policy presets for third-party integrations (Slack, Docker Hub, PyPI, npm, HuggingFace).",
      routing: "Inference routing as security: agent calls hit a local endpoint that transparently proxies to the configured provider. Operators can swap models at runtime, enforce rate limits, and prevent the agent from calling unauthorized model APIs."
    }
  },
  "autoresearchclaw": {
    what: "AutoResearchClaw is a 23-stage autonomous research pipeline that takes a natural language idea and produces a conference-ready paper with real experiments. Its key innovation is ground-truth verification: a VerifiedRegistry whitelists every numeric value from actual experiment runs, and the PaperVerifier rejects any paper containing unverified numbers in results sections. The pipeline is a formal state machine with stage contracts, recursive PIVOT re-execution, multi-agent debate for hypothesis generation, and four-layer citation verification against academic APIs.",
    impact: "Its impact is on anti-fabrication patterns for agent-generated artifacts. The ground-truth registry pattern generalizes beyond research papers: any agent that produces reports, analyses, or dashboards with numbers can use the same approach — whitelist values from actual computation, then verify the final artifact contains only those values. Section-based severity (results sections reject, discussion sections warn) adds context-aware verification.",
    role: "In this map, AutoResearchClaw spans orchestration and eval with indirect harness ties. Orchestration provides the 23-stage pipeline with stage contracts and recursive re-execution. Eval provides ground-truth verification, citation checking, and process reward model scoring. The harness enters through sandboxed experiment execution with immutable harness injection.",
    layerDetails: {
      orchestration: "A 23-stage state machine with StageContracts declaring required inputs, outputs, acceptance criteria, error codes, and max retries. PIVOT decisions trigger recursive re-execution from earlier phases with versioned artifact directories — the pipeline can abandon a hypothesis and restart from synthesis.",
      eval: "Three verification layers: (1) ground-truth registries that whitelist every legitimate value from computation, (2) four-source citation verification (CrossRef → OpenAlex → arXiv → Semantic Scholar) with hallucination detection, (3) process reward model scoring via LLM-as-judge with majority voting."
    }
  },
  "iron-sensor": {
    what: "iron-sensor is an eBPF-based kernel-level behavioral monitor for AI coding agents. It hooks syscall tracepoints (exec, fork, openat, fchmodat) and tracks agent process subtrees via BPF hash maps. A 20+ rule classifier detects persistence writes (cron, systemd, sudoers, shell rc, git hooks), privilege escalation, sensitive file access (SSH keys, /etc/shadow, Docker socket), and suspicious tool usage. Outputs NDJSON for SIEM integration. The entire system is a single static Go binary with embedded eBPF bytecode.",
    impact: "Its impact is framing AI coding agents as a new security surface. Agents inherit full user privileges and can read secrets, write persistence mechanisms, and escalate — the same threat model as an untrusted shell session. The observe-first architecture (detect but don't block) is a deliberate design: behavioral visibility is separable from policy enforcement, and you need the former before you can design the latter.",
    role: "In this map, iron-sensor sits in eval & safety as agent-level observability and guardrails. It also reflects a harness-level concern: the insight that agent credential scope and syscall behavior need monitoring is a constraint that harness designers should internalize.",
    layerDetails: {
      eval: "Kernel-level agent behavioral monitoring via eBPF tracepoints. Process subtree tracking solves the 'agent forks helpers' problem — track the root agent PID, recursively adopt all descendants. Bootstrap scan attaches to already-running agents by walking /proc on startup. First-match-wins rule engine with configurable severity and per-rule overrides.",
      harness: "The indirect harness insight: agents run with full user credentials. SSH keys, cron, systemd units, sudoers are all writable. Harness design should consider credential scoping, filesystem sandboxing, and egress control as first-class concerns, not afterthoughts."
    }
  },
  "turboquant": {
    what: "TurboQuant is a two-stage data-oblivious vector quantization algorithm from Google Research (ICLR 2026) that compresses LLM KV caches to 3-3.5 bits per channel with zero accuracy loss and up to 8x attention speedup. Stage 1 (PolarQuant): applies a random orthogonal rotation, converts to polar coordinates where angular distributions are analytically known, then applies optimal Lloyd-Max scalar quantizers — no per-block normalization constants needed (zero overhead). Stage 2 (QJL): computes the residual error and compresses it to 1 sign bit per dimension via the Quantized Johnson-Lindenstrauss transform, yielding a provably unbiased inner product estimator. The two stages together achieve MSE distortion within 2.7x of information-theoretic lower bounds.",
    impact: "Its impact is on the long-context inference bottleneck. For large models, the KV cache — not model weights — dominates memory at long sequence lengths (Llama-2-70B @ 32K context: 85GB KV cache in FP16 → ~14GB with TurboQuant). This is complementary to weight quantization (GPTQ/AWQ/GGUF): you can compress weights AND KV cache independently. The 6x memory reduction means serving longer contexts or more concurrent users on the same hardware. Zero calibration (data-oblivious) means it applies online, token-by-token, with no preprocessing — fundamentally different from calibration-based methods that require hours of setup. The distinction between weight quantization and KV cache quantization is the key conceptual insight: they are different bottlenecks requiring different techniques.",
    role: "In this map, TurboQuant sits in inference optimization (L04.5) and indirectly affects context (L08) because KV cache size directly constrains effective context length. It represents a new category — runtime activation/cache quantization — distinct from the weight quantization tools already in the map.",
    layerDetails: {
      inference: "Two-stage compression: PolarQuant (random rotation → polar coordinates → Lloyd-Max quantization, zero overhead, b-1 bits) + QJL (1-bit sign residual correction for unbiased inner products). Benchmarked on Llama-3.1-8B-Instruct: 50.06 LongBench at 3.5 bits (matches FP16), 49.44 at 2.5 bits. Perfect Needle-in-Haystack at 3 bits. 8x attention speedup on H100 at 4 bits. JAX implementation with CUDA kernels for QJL stage.",
      context: "KV cache compression directly extends effective context length on fixed hardware. 85GB → 14GB for Llama-2-70B @ 32K means single-GPU deployment for the cache. Orthogonal to architectural approaches (GQA, sliding window) — compresses whatever KV cache the architecture produces."
    }
  },
  "insanely-fast-whisper": {
    what: "Insanely Fast Whisper is a ~130-line CLI wrapper around HF Transformers' automatic-speech-recognition pipeline that correctly composes known optimizations — batched chunking, fp16, and Flash Attention 2 — to transcribe 150 minutes of audio in under 98 seconds on an A100. The entire inference logic is ~15 lines of Python; the value is knowing which flags to set, not writing complex code.",
    impact: "Its impact is a proof that optimization stacking on existing frameworks can yield 19-24x speedups without novel algorithms. The pattern generalizes: batching independent chunks → half precision → fused attention kernels is applicable to any encoder-based model doing chunked inference, not just speech.",
    role: "In this map, Insanely Fast Whisper sits at the intersection of multimodal inference and optimization. It wraps Whisper (L04.4) with inference optimization techniques (L04.5), demonstrating that the serving layer matters as much as the model itself.",
    layerDetails: {
      inference: "Stacks four optimizations: (1) chunked batching (batch_size=24, chunk_length_s=30) for parallel GPU work, (2) fp16 for tensor core throughput, (3) Flash Attention 2 for O(N) memory attention, (4) SDPA fallback for MPS/Mac compatibility. Optional speaker diarization via pyannote as a post-processing alignment pass."
    }
  },
  "openspace": {
    what: "OpenSpace is a self-evolving agent skill engine from HKU Data Science Lab. Agents autonomously learn from every execution, distill reusable skills (structured SKILL.md files with quality metrics), and share evolved skills via a cloud community. The core innovation is a three-trigger, three-mode evolution engine: triggers fire on post-execution analysis, tool degradation detection, and periodic health scans; modes are FIX (in-place repair), DERIVED (enhanced/merged from parents), and CAPTURED (brand-new from novel patterns). Task execution is two-phase: skill-guided attempt first, tool-only fallback if that fails. Skill retrieval uses a multi-stage pipeline: quality filtering → BM25 → embedding reranking → LLM selection.",
    impact: "Its impact is on the token efficiency and compounding intelligence of agent systems. Traditional agents waste tokens re-deriving the same reasoning patterns on similar tasks. OpenSpace's skill reuse yields 54% token savings on repeated task categories, and its GDPVal economic benchmark shows 4.2x performance vs baseline with 72.8% income capture across 44 occupations. The cloud sharing dimension creates a network effect: one agent's improvement upgrades all connected agents. The tool degradation detection (automatic skill evolution when tool success rates drop) addresses the real-world problem of agent skills going stale as APIs change.",
    role: "In this map, OpenSpace spans harness and orchestration. The skill engine, evolution triggers, and quality tracking are harness infrastructure (L06). The two-phase execution strategy and multi-stage retrieval pipeline are orchestration patterns (L07). It indirectly touches eval (GDPVal benchmark, per-skill quality metrics) and context (skill injection into agent prompts).",
    layerDetails: {
      harness: "Three-trigger skill evolution: (1) post-execution analysis via LLM on recorded trajectories, (2) tool degradation detection when success rates drop below 40%, (3) periodic health scans every 5 executions. Three modes: FIX (in-place repair, parent deactivated), DERIVED (enhanced/merged, new directory), CAPTURED (brand-new from novel patterns). Six-level fuzzy patch matching for robust skill application. Anti-loop mechanisms prevent runaway evolution.",
      orchestration: "Two-phase execution: skill-guided (top-K skills injected into system prompt) with tool-only fallback. Multi-stage skill retrieval: quality filtering → BM25 pre-filtering → embedding reranking (text-embedding-3-small) → lexical boost → LLM selection. Mid-iteration skill discovery allows agents to search for additional skills during execution."
    }
  }
};
